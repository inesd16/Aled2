...
class form21 : AppCompatActivity() {
        ...
        button.setOnClickListener(object : View.OnClickListener{
            override fun onClick(v: View?) {
                when (ret1 /*Log.d("ret1", extras?.getInt("val").toString())*/) {
                    1 -> {
                        saveData()
                        writeFile(text121.text.toString(), text221.text.toString(), text321.text.toString(), text421.text.toString(), this)
                        read()
    ...
    fun saveData(){
        ...
        jsonObj.put("reponse 1", text121.text.toString())
        jsonObj.put("reponse 2", text221.text.toString())
        jsonObj.put("reponse 3", text321.text.toString())
        jsonObj.put("reponse 4", text421.text.toString())
        ...
        with(sharedNewAnswer.edit()) {
            putString("userResponse21", jsonObj.toString())
            commit()
        }
    }
    ...
    fun read() {
        requestPermission(Manifest.permission.READ_EXTERNAL_STORAGE, readESRequestCode) {
            ...
            val file = File(dir, "answerForm21.txt")
