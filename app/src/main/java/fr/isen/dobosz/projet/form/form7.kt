package fr.isen.dobosz.projet.form

import android.Manifest
import android.content.Context
import android.content.Intent
import android.content.pm.PackageManager
import android.os.Build
import android.os.Bundle
import android.os.Environment
import android.util.Log
import android.view.View
import androidx.appcompat.app.AppCompatActivity
import androidx.core.app.ActivityCompat
import androidx.core.content.ContextCompat
import fr.isen.dobosz.projet.R
import fr.isen.dobosz.projet.StarActivity
import kotlinx.android.synthetic.main.activity_form7.*
import kotlinx.android.synthetic.main.activity_form7.button
import org.json.JSONArray
import org.json.JSONObject
import java.io.File
import java.io.FileNotFoundException
import java.io.IOException

class form7 : AppCompatActivity() {
    var nmbOfMoveOnStarImage:Int = 0
    var posArray = ArrayList<String>()
    companion object{
        var writeESRequestCode = 9
        var readESRequestCode = 10
    }
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_form7)
        if(isWriteStoragePermissionGranted()){
            requestPermission(Manifest.permission.WRITE_EXTERNAL_STORAGE, StarActivity.writeESRequestCode) {
            }
        }
        if(isReadStoragePermissionGranted()){
            requestPermission(Manifest.permission.READ_EXTERNAL_STORAGE, StarActivity.readESRequestCode) {
            }
        }
        val form: Intent ?= intent
        val extras: Bundle ?= form?.extras
        button.setOnClickListener(object : View.OnClickListener{
            override fun onClick(v: View?) {
                when (Log.d("ret1", extras?.getInt("val").toString())) {
                    1 -> {
                        saveData()
                        writeFile(text17.text.toString(), text27.text.toString(), text37.text.toString(), this)
                        read()
                        val form1Activity = Intent(this@form7, form1::class.java)
                        form1Activity.putExtra("val", 0)
                        startActivity(form1Activity)
                    }
                    2 -> {
                        saveData()
                        writeFile(text17.text.toString(), text27.text.toString(), text37.text.toString(), this)
                        read()
                        val form2Activity = Intent(this@form7, form2::class.java)
                        form2Activity.putExtra("val", 0)
                        startActivity(form2Activity)
                    }
                    3 -> {
                        saveData()
                        writeFile(text17.text.toString(), text27.text.toString(), text37.text.toString(), this)
                        read()
                        val form3Activity = Intent(this@form7, form3::class.java)
                        form3Activity.putExtra("val", 0)
                        startActivity(form3Activity)
                    }
                    4 -> {
                        saveData()
                        writeFile(text17.text.toString(), text27.text.toString(), text37.text.toString(), this)
                        read()
                        val form4Activity = Intent(this@form7, form4::class.java)
                        form4Activity.putExtra("val", 0)
                        startActivity(form4Activity)
                    }
                    5 -> {
                        saveData()
                        writeFile(text17.text.toString(), text27.text.toString(), text37.text.toString(), this)
                        read()
                        val form5Activity = Intent(this@form7, form5::class.java)
                        form5Activity.putExtra("val", 0)
                        startActivity(form5Activity)
                    }
                    6 -> {
                        saveData()
                        writeFile(text17.text.toString(), text27.text.toString(), text37.text.toString(), this)
                        read()
                        val form6Activity = Intent(this@form7, form6::class.java)
                        form6Activity.putExtra("val", 0)
                        startActivity(form6Activity)
                    }
                    7 -> {
                        saveData()
                        writeFile(text17.text.toString(), text27.text.toString(), text37.text.toString(), this)
                        read()
                        val form7Activity = Intent(this@form7, form7::class.java)
                        form7Activity.putExtra("val", 0)
                        startActivity(form7Activity)
                    }
                    8 -> {
                        saveData()
                        writeFile(text17.text.toString(), text27.text.toString(), text37.text.toString(), this)
                        read()
                        val form8Activity = Intent(this@form7, form8::class.java)
                        form8Activity.putExtra("val", 0)
                        startActivity(form8Activity)
                    }
                    9 -> {
                        saveData()
                        writeFile(text17.text.toString(), text27.text.toString(), text37.text.toString(), this)
                        read()
                        val form9Activity = Intent(this@form7, form9::class.java)
                        form9Activity.putExtra("val", 0)
                        startActivity(form9Activity)
                    }
                    10 -> {
                        saveData()
                        writeFile(text17.text.toString(), text27.text.toString(), text37.text.toString(), this)
                        read()
                        val form10Activity = Intent(this@form7, form10::class.java)
                        form10Activity.putExtra("val", 0)
                        startActivity(form10Activity)
                    }
                    11 -> {
                        saveData()
                        writeFile(text17.text.toString(), text27.text.toString(), text37.text.toString(), this)
                        read()
                        val form11Activity = Intent(this@form7, form11::class.java)
                        form11Activity.putExtra("val", 0)
                        startActivity(form11Activity)
                    }
                    12 -> {
                        saveData()
                        writeFile(text17.text.toString(), text27.text.toString(), text37.text.toString(), this)
                        read()
                        val form12Activity = Intent(this@form7, form12::class.java)
                        form12Activity.putExtra("val", 0)
                        startActivity(form12Activity)
                    }
                    13 -> {
                        saveData()
                        writeFile(text17.text.toString(), text27.text.toString(), text37.text.toString(), this)
                        read()
                        val form13Activity = Intent(this@form7, form13::class.java)
                        form13Activity.putExtra("val", 0)
                        startActivity(form13Activity)
                    }
                    14 -> {
                        saveData()
                        writeFile(text17.text.toString(), text27.text.toString(), text37.text.toString(), this)
                        read()
                        val form14Activity = Intent(this@form7, form14::class.java)
                        form14Activity.putExtra("val", 0)
                        startActivity(form14Activity)
                    }
                    15 -> {
                        saveData()
                        writeFile(text17.text.toString(), text27.text.toString(), text37.text.toString(), this)
                        read()
                        val form15Activity = Intent(this@form7, form15::class.java)
                        form15Activity.putExtra("val", 0)
                        startActivity(form15Activity)
                    }
                    16 -> {
                        saveData()
                        writeFile(text17.text.toString(), text27.text.toString(), text37.text.toString(), this)
                        read()
                        val form16Activity = Intent(this@form7, form16::class.java)
                        form16Activity.putExtra("val", 0)
                        startActivity(form16Activity)
                    }
                    17 -> {
                        saveData()
                        writeFile(text17.text.toString(), text27.text.toString(), text37.text.toString(), this)
                        read()
                        val form17Activity = Intent(this@form7, form17::class.java)
                        form17Activity.putExtra("val", 0)
                        startActivity(form17Activity)
                    }
                    18 -> {
                        saveData()
                        writeFile(text17.text.toString(), text27.text.toString(), text37.text.toString(), this)
                        read()
                        val form18Activity = Intent(this@form7, form18::class.java)
                        form18Activity.putExtra("val", 0)
                        startActivity(form18Activity)
                    }
                    19 -> {
                        saveData()
                        writeFile(text17.text.toString(), text27.text.toString(), text37.text.toString(), this)
                        read()
                        val form19Activity = Intent(this@form7, form19::class.java)
                        form19Activity.putExtra("val", 0)
                        startActivity(form19Activity)
                    }
                    20 -> {
                        saveData()
                        writeFile(text17.text.toString(), text27.text.toString(), text37.text.toString(), this)
                        read()
                        val form20Activity = Intent(this@form7, form20::class.java)
                        form20Activity.putExtra("val", 0)
                        startActivity(form20Activity)
                    }
                    21 -> {
                        saveData()
                        writeFile(text17.text.toString(), text27.text.toString(), text37.text.toString(), this)
                        read()
                        val form21Activity = Intent(this@form7, form21::class.java)
                        form21Activity.putExtra("val", 0)
                        startActivity(form21Activity)
                    }
                    22 -> {
                        saveData()
                        writeFile(text17.text.toString(), text27.text.toString(), text37.text.toString(), this)
                        read()
                        val form22Activity = Intent(this@form7, form22::class.java)
                        form22Activity.putExtra("val", 0)
                        startActivity(form22Activity)
                    }
                    else -> {
                    }
                }
            }
        })
    }

    fun saveData(){
        val jsonObj = JSONObject()
        val jsonArray = JSONArray()
        jsonObj.put("reponse 1", text17.text.toString())
        jsonObj.put("reponse 2", text27.text.toString())
        jsonObj.put("reponse 3", text37.text.toString())
        jsonArray.put(jsonObj)
        posArray.add(nmbOfMoveOnStarImage++,jsonArray.toString())
        val sharedNewAnswer = this.getSharedPreferences("sharedNewAnswer", Context.MODE_PRIVATE) ?: return
        with(sharedNewAnswer.edit()) {
            putString("userResponse7", jsonObj.toString())
            commit()
        }
    }

    fun read() {
        requestPermission(Manifest.permission.READ_EXTERNAL_STORAGE, readESRequestCode) {
            val root = getExternalFilesDir("DataToSend")
            val dir = File(root!!.absolutePath)
            val file = File(dir, "answerForm7.txt")
            val read = file.readText()
            System.out.println("VALEUR READ " + read)
            //Read text from file
            // val iStream = openFileInput(dir.toString())
        }
    }

    fun writeFile(coords1: String, coords2: String, coords3: String, context: View.OnClickListener){
        val state = Environment.getExternalStorageState()
        var success = true
        val root = getExternalFilesDir("DataToSend")
        if(Environment.MEDIA_MOUNTED.equals((state))){
            val dir = File(root!!.absolutePath)
            System.out.println(dir.toString())
            if(!dir.exists()){
                success = dir.mkdir()
                System.out.println("does not exists yet")
            }
            else{
                System.out.println("exists")
            }
            if (success) {
                val file = File(dir, "answerForm7.txt")
                System.out.println("success true")
                try {
                    file.createNewFile()
                    posArray.add(coords1)
                    posArray.add(coords2)
                    posArray.add(coords3)
                    file.writeText(posArray.toString())
                    System.out.println("SAVED")
                } catch (e: FileNotFoundException) {
                    System.out.println("E1")
                    e.printStackTrace()
                } catch (e: IOException) {
                    System.out.println("E2")
                    e.printStackTrace()
                }
            }
        }
    }

    fun requestPermission(permissionToRequest: String, requestCode: Int, handler: ()-> Unit) {
        if(ContextCompat.checkSelfPermission(this, permissionToRequest) != PackageManager.PERMISSION_GRANTED) {
            if(ActivityCompat.shouldShowRequestPermissionRationale(this, permissionToRequest)) {
                //display toast
            } else {
                ActivityCompat.requestPermissions(this, arrayOf(permissionToRequest), requestCode)
            }
        } else {
            handler()
        }
    }

    override fun onRequestPermissionsResult(requestCode: Int,permissions: Array<out String>, grantResults: IntArray) {
        super.onRequestPermissionsResult(requestCode, permissions, grantResults)
        when (requestCode) {
            9 -> {
                Log.d("TAG", "External storage2")
                if (grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                    Log.v("TAG", "Permission: " + permissions[0] + "was " + grantResults[0])
                    //resume tasks needing this permission
                    //downloadPdfFile();
                } else {
                    //progress.dismiss();
                }
            }
            10 -> {
                Log.d("TAG", "External storage1")
                if (grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                    Log.v("TAG", "Permission: " + permissions[0] + "was " + grantResults[0])
                    //resume tasks needing this permission
                    //SharePdfFile();
                } else {
                    //progress.dismiss();
                }
            }
        }
    }

    fun isReadStoragePermissionGranted():Boolean {
        if (Build.VERSION.SDK_INT >= 23) {
            if (checkSelfPermission(android.Manifest.permission.READ_EXTERNAL_STORAGE)
                == PackageManager.PERMISSION_GRANTED) {
                Log.v("TAG","Permission is granted1")
                return true
            } else {

                Log.v("TAG","Permission is revoked1")
                ActivityCompat.requestPermissions(this, arrayOf(android.Manifest.permission.READ_EXTERNAL_STORAGE),
                    StarActivity.readESRequestCode
                )
                return false
            }
        }
        else { //permission is automatically granted on sdk<23 upon installation
            Log.v("TAG","Permission is granted1")
            return true
        }
    }

    fun isWriteStoragePermissionGranted():Boolean {
        if (Build.VERSION.SDK_INT >= 23) {
            if (checkSelfPermission(android.Manifest.permission.WRITE_EXTERNAL_STORAGE)
                == PackageManager.PERMISSION_GRANTED) {
                Log.v("TAG","Permission is granted2")
                return true
            } else {

                Log.v("TAG","Permission is revoked2")
                ActivityCompat.requestPermissions(this, arrayOf(android.Manifest.permission.WRITE_EXTERNAL_STORAGE),
                    StarActivity.writeESRequestCode
                );
                return false
            }
        }
        else { //permission is automatically granted on sdk<23 upon installation
            Log.v("TAG","Permission is granted2")
            return true
        }
    }
}
